...
class Firework:
    def __init__(self, canvas, x, y, global_crackle_list, flash_callback, instant_detonate=False):
        ...
        # Pull dynamic canvas height to ground fountains properly
...
class Skyline:
    ...
    def resize(self, width, height):
        """Wipes old tracking parameters and rebuilds city layout matching new scale."""
        self.width = width
        self.height = height
    ...
    def draw(self, canvas):
        # Anchor Moon dynamically to top right edge
...
class TextOverlay:
    ...
    def draw(self, canvas):
        # Read absolute midpoints dynamically from active window
...
def on_window_resize(event):
    """Fires dynamically when window dimensions scale, protecting skyline ratios."""
    # Ensure event comes from canvas item, ignoring nested widget layouts
...
def run_show(canvas, fireworks, crackles, skyline, flashes, flag, overlay):
    ...
    # 1. Core Structural Backdrops
    flag.draw()
    skyline.draw(canvas)
    overlay.update()
    overlay.draw(canvas)
    
    # 2. Rendering Physics Calculations
    for f in flashes[:]:
        f['radius'] += 6
    ...
    # 3. Dynamic Launch Spawning Adaptability 
    if random.random() < 0.05:
        w = canvas.winfo_width()
        h = canvas.winfo_height()
        # Fallbacks check to prevent launch glitches before canvas instantiates tracking limits
...
# Packaging layout using expand parameters lets Tkinter manage scaling logic smoothly
canvas = tk.Canvas(root, bg='#03030d', width=800, height=580, highlightthickness=0)
canvas.pack(fill=tk.BOTH, expand=True)

# Event Listener Mappings
canvas.bind("<Button-1>", on_canvas_click)
canvas.bind("<Configure>", on_window_resize) # Catches window resize triggers dynamically
