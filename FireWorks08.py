...
# STATUS: Research
# VERSION: 0.0.0
# NOTES: A.I. Generated - but not working.
# DATE: 2026-05-19 05:17:28
# FILE: FireWorks08.py
...
class Firework:
    def __init__(self, canvas, x, y, global_crackle_list, flash_callback, shell_type=None, instant_detonate=False):
        ...
        # Ground level if fountain and not clicked
    ...
    def draw(self):
        if not self.exploded:
            ...
            for p in self.particles:
                if p['life'] > 0:
                    # FIXED: Corrected conditional check to handle all shell type aesthetics safely
...
def on_canvas_click(event):
    fireworks.append(
        Firework(canvas, event.x, event.y, crackles, trigger_flash, shell_type=selected_type, instant_detonate=True)
    )

def set_shell_type(event):
    global selected_type
    key = event.char
    if key == '1': selected_type = 'classic'
    elif key == '2': selected_type = 'willow'
    elif key == '3': selected_type = 'ring'
    elif key == '4': selected_type = 'fountain'
...
def run_show(canvas, fireworks, crackles, skyline, flashes, flag):
    ...
    hud_text = f"Selected Shell: {selected_type.upper()}\nHotkeys: [1] Classic  [2] Willow  [3] Ring  [4] Fountain"
    canvas.create_text(30, 210, anchor=tk.NW, text=hud_text, fill="#aaaaaa", font=("Arial", 10, "bold"))
...
root.bind("1", set_shell_type)
root.bind("2", set_shell_type)
root.bind("3", set_shell_type)
root.bind("4", set_shell_type)
root.focus_set()  
