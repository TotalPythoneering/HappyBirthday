...
class Firework:
    def __init__(self, canvas, x, y, global_crackle_list, flash_callback, shell_type=None, instant_detonate=False):
        self.canvas = canvas
        self.x = x
        # If it's a fountain and not manually clicked, launch from ground level to simulate a fountain box
        self.shell_type = shell_type if shell_type else random.choice(['classic', 'willow', 'ring', 'fountain'])
        self.y = 550 if (self.shell_type == 'fountain' and not instant_detonate) else y
        ...
        if instant_detonate or self.shell_type == 'fountain':
            # Fountains start exploding immediately without an initial rocket ascent
            self.exploded = True
            self.explode()
        else:
            self.exploded = False
    ...
    def update(self):
        if not self.exploded:
            ...
            for p in self.particles:
                ...
                # Apply custom physics depending on shell type
                if self.shell_type == 'willow':
                    p['vy'] += 0.15  
                    p['vx'] *= 0.98  
                elif self.shell_type == 'fountain':
                    p['vy'] += 0.22  # Stronger gravity pulls waterfall back down quickly
                    p['vx'] *= 0.97  # Air resistance creates a narrower spray pattern
                else:
                    p['vy'] += 0.06  
                
                p['life'] -= 1

                # Fountain & Willow trail effects
        ...
        if not self.exploded:
            ...
            for p in self.particles:
                if p['life'] > 0:
                    ...
                    # Size tuning
                    if self.shell_type == 'fountain':
                        size = random.choice([1, 2])  # Shimmering mixed sizes
                    else:
    ...
    def explode(self):
        ...
        if self.shell_type == 'classic':
            ...
            # Electric white, bright blue, or gold fountains look best
            self.color = random.choice(['#ffffff', '#2affff', '#ffca3a'])
            # Shoot a dense volley upward within a narrow angled cone (-60 to -120 degrees)
            for _ in range(80):
                angle = random.uniform(-1.9, -1.2)  # Upward trajectory mapping
                speed = random.uniform(6, 14)       # High velocity gradient for tiered vertical spray
                self.particles.append({
                    'x': self.x, 
                    'y': self.y, 
                    'vx': math.cos(angle) * speed, 
                    'vy': math.sin(angle) * speed, 
                    'life': random.randint(40, 65)  # Extended lifespan to fall back down safely
                })
...
class Skyline:
    ...
    def draw(self, canvas):
        canvas.create_oval(700, 40, 760, 100, fill="#fffebb", outline="")
...
def trigger_finale():
    for _ in range(16):
        ...
        root.after(delay, lambda: fireworks.append(
            Firework(canvas, random.randint(60, 740), 550, crackles, trigger_flash)
        ))

def on_canvas_click(event):
    # Left click now defaults to creating a fountain effect right at your cursor position!
    fireworks.append(
        Firework(canvas, event.x, event.y, crackles, trigger_flash, shell_type='fountain', instant_detonate=True)
    )

def run_show(canvas, fireworks, crackles, skyline, flashes, flag):
    canvas.delete("all")
    ...
    # Ambient automatic launches can pick any of the 4 shell types
    if random.random() < 0.05:
        x = random.randint(120, 680)
        fireworks.append(Firework(canvas, x, 550, crackles, trigger_flash))

    canvas.after(20, run_show, canvas, fireworks, crackles, skyline, flashes, flag)
...
run_show(canvas, fireworks, crackles, skyline, flashes, flag)
root.mainloop()
