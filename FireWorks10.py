...
class TextOverlay:
    """Manages the shifting color and drop-shadow styling of the greeting."""
    ...
    def update(self):
        self.tick += 1
        if self.tick % 25 == 0:  # Shift colors smoothly every 25 frames
            self.color_index = (self.color_index + 1) % len(self.colors)

    def draw(self):
        current_color = self.colors[self.color_index]
        # Dark silhouette drop-shadow offset
        self.canvas.create_text(self.x + 3, self.y + 3, text=self.text, fill="#040410", font=("Impact", 44, "bold"), justify=tk.CENTER)
        # Main shifting text layer
        self.canvas.create_text(self.x, self.y, text=self.text, fill=current_color, font=("Impact", 44, "bold"), justify=tk.CENTER)
...
def run_show(canvas, fireworks, crackles, skyline, flashes, flag, overlay):
    ...
    # 2. Text Overlay (drawn in mid-ground so fireworks bloom in front and trails pass behind)
    overlay.update()
    overlay.draw()
...
overlay = TextOverlay(canvas, "Happy 4th of July!", width=800, height=580)
